...
17. [ClaudeRuntime との統合](#clauderuntime-との統合)
18. [ClaudeTestKit との連携](#claudetestkit-との連携)
19. [後方互換性](#後方互換性)
...
- **TaggingRules キャッシュ**: テスト間でキャッシュが残留しないよう、各テストケースの setUp/tearDown で `NBHistoryCacheClear[]` を呼び出すことを推奨します。
...
ClaudeRuntime および ClaudeTestKit の導入にともなう NBAccess の変更は、既存のコードとの後方互換性を完全に維持するよう設計されています。
...
ClaudeRuntime 連携のために追加された以下の API 群は、既存の API に依存しない独立した拡張です。
