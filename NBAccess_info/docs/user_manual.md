...
15. [SourceVault との統合とパス参照 API](#sourcevault-との統合とパス参照-api)
16. [その他のユーティリティ](#その他のユーティリティ)
17. [[実験的] ノートブックファイルのセル操作](#実験的-ノートブックファイルのセル操作)
18. [ClaudeRuntime との統合](#clauderuntime-との統合)
19. [ClaudeTestKit との連携](#claudetestkit-との連携)
20. [後方互換性](#後方互換性)
...
[SourceVault](https://github.com/transreal/SourceVault) は、複数 PC・複数ボールト構成の中でファイルパスを抽象化して同一性 (identity) を判定するためのパッケージです。NBAccess は SourceVault と連携することで、ノートブックに保存されるアクセス可能パス情報を**特定 PC の絶対パスに依存しないシンボリックな参照 (PathRef)**として保持できます。

### 導入の背景

従来の NBAccess では、Claude Code が参照可能なディレクトリを `claudecode-accessible-dirs` キーに**絶対パス文字列**として保存していました。この方式では以下のような問題が生じます。

- 別 PC でノートブックを開いた場合、絶対パスが解決できず Claude Code がファイルを読めない。
- Dropbox のような同期型ボールトでも、ユーザー名や OneDrive ルートが PC ごとに異なるため、同一ファイルが別パスとして扱われる。
- 履歴やアタッチメントに保存されたパスが、PC をまたぐと再現性を失う。

SourceVault は、こうしたパスを**ボールトルート + 相対パス**の組として正規化することでこの問題を解決します。NBAccess の PathRef API は SourceVault がロードされている場合にこのシンボリック化を利用します。SourceVault が未ロードでも従来通り絶対パスベースで動作します（フォールバック）。

### 用語整理

| 用語 | 意味 |
|---|---|
| **PathRef** | パスの同一性情報を持つ Association。`NBNormalizePath[path]` の返り値。 |
| **AccessPathRef** | PathRef にアクセスモード・送信可否を加えたエントリ。`{<|"PathRef" -> ..., "Mode" -> ..., "CloudSend" -> ...|>, ...}` |
| **SymbolicPath** | `{"$onWork", "subdir", ...}` のようなボールトルート相対のリスト表現。 |
| **PhysicalPath** | 現 PC 上で実際に存在する絶対パス文字列。 |
| **ResolutionStatus** | `ResolvedOnThisPC` / `AliasOnly` / `Unrooted` のいずれか。 |

### Rule 104: 同一性と権限の分離

SourceVault 連携で重要な原則として **rule 104** があります。

> **PathRef や AccessPathRef の保存自体が権限を与えるわけではない。**
> 実権限は必ず `PhysicalPath` を現 PC で解決し、実在を確認した上で `Mode` と `CloudSend` (privacy) を見て判定する。

別 PC のエイリアスにのみ一致するパスは、参照情報としてノートブックに保持されても、現 PC では「未解決 (alias-only)」として扱われ、settings.json などにマテリアライズされません。これにより、誤って他 PC 用のパスがアクセス許可されることを防ぎます。

### NBNormalizePath

絶対パス文字列を、複数 PC 間で安定な PathRef Association に正規化します。

```mathematica
NBNormalizePath["C:\\Users\\imai_\\Dropbox\\work\\notes.txt"]
(* <|
     "Kind"             -> "File",
     "RootId"           -> "$onWork",
     "Parts"            -> {"notes.txt"},
     "SymbolicPath"     -> {"$onWork", "notes.txt"},
     "PhysicalPath"     -> "C:\\Users\\imai_\\Dropbox\\work\\notes.txt",
     "ResolutionStatus" -> "ResolvedOnThisPC",
     "MatchedBy"        -> "LocalRoot"
   |> *)
```

`ResolutionStatus` フィールドの値:

| 値 | 意味 |
|---|---|
| `"ResolvedOnThisPC"` | 現 PC の実ボールトルート配下に存在する。 |
| `"AliasOnly"` | 別 PC のエイリアスにのみ一致。現 PC では実在しない。 |
| `"Unrooted"` | どのボールトルートにも該当しない（プロジェクト管理外）。 |

`MatchedBy` フィールド: `"LocalRoot"` | `"Alias"` | `"None"`。

SourceVault が未ロードの場合は、SymbolicPath を持たないシンプルな PathRef にフォールバックします。

### NBResolvePathRef

PathRef や `{"$onWork", ...}` 形式の SymbolicPath を、現 PC の絶対パス文字列に解決します。

```mathematica
NBResolvePathRef[<|"SymbolicPath" -> {"$onWork", "notes.txt"}|>]
(* "C:\\Users\\imai_\\Dropbox\\work\\notes.txt" — 現 PC で解決できた場合 *)

NBResolvePathRef[{"$onOther", "secret.txt"}]
(* Missing["NotResolvable", ...] — 現 PC にないルートを参照している *)
```

解決できない場合（ルート未定義、エイリアスのみ一致など）は `Missing[...]` を返します。settings.json などへの materialize には、解決できたものだけを使用してください。

### AccessPathRef とそのリスト管理

Claude Code が参照可能なディレクトリは、`AccessPathRef` Association のリストとして保存されます。

```mathematica
<|
  "PathRef"   -> NBNormalizePath["C:\\Users\\imai_\\Dropbox\\project"],
  "Mode"      -> "Read",      (* "List" | "Read" | "ReadWrite" *)
  "CloudSend" -> "Ask"        (* False | True | "Ask" *)
|>
```

| フィールド | 値の意味 |
|---|---|
| `"Mode"` | `"List"` — 一覧表示のみ可。`"Read"` — 内容読み取り可。`"ReadWrite"` — 書き込みも可。 |
| `"CloudSend"` | `False` — クラウド LLM に送らない。`True` — 自動送信可。`"Ask"` — 都度確認。 |

### NBSetAccessiblePathRefs

ノートブックの TaggingRules (`claudeAccessiblePathRefs` キー) に AccessPathRef リストを保存します。**これが Phase 2 以降の正本 (canonical)** となります。旧キー `claudecode-accessible-dirs` は read fallback としてのみ残ります。
...
NBSetAccessiblePathRefs[nb,
  {
    <|"PathRef"   -> NBNormalizePath["C:\\Users\\imai_\\Dropbox\\project"],
      "Mode"      -> "ReadWrite",
      "CloudSend" -> False|>,
    <|"PathRef"   -> NBNormalizePath["C:\\data\\public"],
      "Mode"      -> "Read",
      "CloudSend" -> True|>
  }]

(* nb を省略すると EvaluationNotebook[] に保存 *)
NBSetAccessiblePathRefs[refs]
```

### NBGetAccessiblePathRefs

ノートブックから AccessPathRef リストを取得します。`claudeAccessiblePathRefs` が無い旧ノートブックでは、`claudecode-accessible-dirs` を AccessPathRef 形式に変換して返します（read fallback）。

```mathematica
NBGetAccessiblePathRefs[nb]
(* {<|"PathRef" -> ..., "Mode" -> "Read", "CloudSend" -> "Ask"|>, ...} *)

NBGetAccessiblePathRefs[]
(* EvaluationNotebook[] から取得 *)
```

返り値は canonical な参照リストで、ノートブックに永続化される情報です。実パスへの解決は `NBResolvePathRef` または `NBGetAccessibleDirs` で行います。

### NBNormalizeAccessPathRef

絶対パス文字列または部分指定の AccessPathRef を、完全な AccessPathRef Association に正規化します。`NBSetAccessibleDirs` から `NBSetAccessiblePathRefs` への内部変換でも利用されます。

```mathematica
NBNormalizeAccessPathRef["C:\\data\\public"]
(* <|"PathRef" -> ..., "Mode" -> "Read", "CloudSend" -> "Ask"|>
   — 文字列指定時のデフォルトは Mode="Read", CloudSend="Ask" *)

NBNormalizeAccessPathRef[<|"PathRef" -> ref, "Mode" -> "ReadWrite"|>]
(* 不足キー (CloudSend) は既定値で補完 *)
```

### NBSetSnapshotPrivacyLevel

SourceVault が管理する snapshot に対し、PrivacyLevel を直接書き換えるための高権限 API です。通常は各セルから自動導出されますが、人間の判断で上書きしたい場合に使用します。

```mathematica
NBSetSnapshotPrivacyLevel["snapshot-xyz", 1.0]
```

`$NBApprovalHeads` に登録されており、LLM 経由の呼び出しでは人間の承認が必要です。

### Snapshot ベースの NBFileSpec

`NBFileSpec` には snapshot レイヤーが導入されており、同じパスに対する繰り返し問い合わせはキャッシュから即座に返されます。SourceVault のスナップショット更新時には、`NBFileSpecCacheClear[]` でキャッシュを破棄してください。

```mathematica
(* NBFileSpec の内部キャッシュ（base / projection）をクリア *)
...
| 旧 API | 推奨 |
|---|---|
| `NBSetAccessibleDirs[nb, dirs]` | `NBSetAccessiblePathRefs[nb, refs]` を推奨。文字列を渡せば内部で自動的に AccessPathRef に正規化されます。 |
| `NBGetAccessibleDirs[nb]` | 実 PC の絶対パスが欲しい場合は引き続き使用可。canonical 情報の取得は `NBGetAccessiblePathRefs[nb]`。 |
| `claudecode-accessible-dirs` キー | Phase 2 以降は read fallback のみ。新規書き込みは `claudeAccessiblePathRefs` を使用。 |

SourceVault が未ロードでも、旧 API は従来どおり絶対パスベースで動作します。SourceVault をロードすることで、これらの API は自動的に PathRef を介したクロス PC 解決を行うようになります。
...
### アクセス可能ディレクトリ（絶対パスベース API）

Claude Code が参照可能なディレクトリリストを TaggingRules に保存・取得する旧来 API です。`nb` を省略すると `EvaluationNotebook[]` が使用されます。
...
SourceVault がロードされている場合、`NBSetAccessibleDirs` の内部では `NBNormalizeAccessPathRef` 経由で AccessPathRef に変換され、`claudeAccessiblePathRefs` キーにも反映されます。`NBGetAccessibleDirs` は現 PC で解決できる物理パスのみを返します（rule 104）。

クロス PC で安定して動作させたい場合は、新しい `NBSetAccessiblePathRefs` / `NBGetAccessiblePathRefs` を直接利用することを推奨します。詳しくは [SourceVault との統合とパス参照 API](#sourcevault-との統合とパス参照-api) を参照してください。
...
SourceVault がロードされている場合、`NBFileSpec` の内部結果は snapshot ベースでキャッシュされます。スナップショット更新時には `NBFileSpecCacheClear[]` でキャッシュを破棄してください。
...
- **SourceVault 連携 API の追加**: `NBNormalizePath`、`NBResolvePathRef`、`NBSetAccessiblePathRefs`、`NBGetAccessiblePathRefs`、`NBNormalizeAccessPathRef`、`NBSetSnapshotPrivacyLevel`、`NBFileSpecCacheClear` が追加されました。複数 PC 間でのアクセス可能パスのシンボリック管理と snapshot ベースキャッシュを実現します。
- **`NBMarkCellConfidential` の PrivacyLevel 指定対応**: 第3引数で PrivacyLevel 数値 (0.0〜1.0) を直接指定できるようになりました。`$NBApprovalHeads` に登録されており、LLM 経由の呼び出しでは人間の承認ゲートが起動します。
...
従来の NBAccess は `AccessLevel` 数値によるプライバシー制御を主体としていましたが、ClaudeRuntime との連携により以下の要件が生じました。

- **式レベルの実行制御**: LLM が生成したコードを実行する前に、許可されたヘッド・操作かどうかを検証する必要があります。
- **情報フロー追跡**: 機密データが許可されていない出力先（クラウド LLM 等）に漏洩しないよう、ラベルによる流れの管理が必要です。
- **関数単位のポリシー**: 個々の関数に実行ポリシーおよびリリースポリシーを登録し、細粒度のアクセス制御を実現します。

### 実行検証 API

LLM が生成した式を安全に実行するための API です。
...
検証済みの式を安全に実行し、結果を返します。内部で `NBValidateHeldExpr` → 実行 → `NBRedactExecutionResult` の順で処理します。

```mathematica
result = NBExecuteHeldExpr[HoldComplete[1 + 1], accessSpec]
(* <|"Success" -> True, "RawResult" -> 2, "Error" -> None|> *)
```

#### NBRedactExecutionResult

実行結果から機密情報を除去し、安全な形式に変換します。`AccessSpec` の `SinkLabel` に従い、リリース可能なデータのみを返します。
...
`NBAuthorize` は3つのゲートを統合した認可判定を行います。
...
| 関数 | 役割 |
|---|---|
| `NBPolicyGate[obj, req]` | 半順序ラベルに基づく情報フロー判定を行います。`PolicyLabel` / `ContainerLabel` / `SinkLabel` を考慮します。 |
| `NBScoreGate[obj, req]` | 数値スコアに基づくルーティング判定を行います。advisory 体系であり、permit/deny の主体ではありません。 |
| `NBEnvironmentGate[obj, req]` | 実行環境・Sink・Principal に基づく制約チェックを行います。 |

### ラベル代数 API

NBAccess はラベル（セキュリティ分類）の代数的操作を提供します。将来的にはラベルが数値アクセスレベルに代わるアクセス制御の主体系となる予定です。現バージョンでは数値スコアは advisory 体系として併存しています。

```mathematica
NBLabelQ[label]                (* 有効なラベルか判定します *)
NBLabelBottom[]                (* 最小制約ラベル (public) を返します *)
NBLabelTop[]                   (* 最大制約ラベル (全拒否) を返します *)
NBLabelJoin[l1, l2]            (* より制約的なラベルを返します (join) *)
NBLabelMeet[l1, l2]            (* より緩いラベルを返します (meet) *)
NBLabelLEQ[l1, l2]             (* l1 から l2 への情報フローが許可されるか判定します *)
NBCanFlowToQ[srcLabel, dstLabel]             (* flow が許可されるか判定します *)
NBCanDeclassifyQ[srcLabel, dstLabel, req]    (* declassify が正当か判定します *)
NBEffectiveLabel[obj, req]                   (* 実効ラベルを計算します *)
```

### Principal・委任 API

```mathematica
NBRegisterPrincipal[name, opts]   (* アクセス主体を登録します *)
NBGrantActsFor[p, q]              (* principal p が q として行動できる委任を登録します *)
NBActsForQ[p, q]                  (* p が q として行動可能か判定します *)
```

### 関数セキュリティ API

個々の関数にセキュリティメタデータを登録します。

```mathematica
NBRegisterFunctionSecurity[myFunc, <|
  "DefinitionLabel" -> NBLabelBottom[],
  "ExecPolicy"      -> "Guarded",
  "ReleasePolicy"   -> <|...|>
|>]
```

登録後は以下の API でメタデータを参照できます。

| 関数 | 返り値 |
|---|---|
| `NBFunctionDefinitionLabel[f]` | 関数定義の閲覧可否を制御するラベルを返します。 |
...
```mathematica
GuardedApply[req, myFunc, {arg1, arg2}]
```

#### Declassify

オブジェクトのラベルを `releaseSpec` に従って引き下げます。`req` の Principal が acts-for 権限を持つ場合のみ許可されます。
...
LLM が実行可能なヘッドのリストをカテゴリ単位で管理します。

```mathematica
$NBAllowedHeads          (* 実行可能ヘッドのリスト *)
$NBApprovalHeads         (* 承認を要するヘッドのリスト *)
$NBDenyHeads             (* 常に拒否するヘッドのリスト *)
$NBAllowedHeadsByCategory   (* カテゴリ別の許可ヘッドリスト (Association) *)
$NBDisabledCategories       (* 無効化されたカテゴリの追跡 (Association) *)

NBEnableCategory["Arithmetic"]     (* カテゴリを有効化します *)
NBDisableCategory["FileIO"]        (* カテゴリを無効化します *)
NBCategoryEnabled["Arithmetic"]    (* カテゴリが有効かを返します *)
```

カテゴリの有効/無効切り替えは `$NBAllowedHeadsByCategory` の対応エントリを動的に `$NBAllowedHeads` に反映します。

`$NBApprovalHeads` には `NBMarkCellConfidential` および `NBSetSnapshotPrivacyLevel` も登録されており、LLM 経由でこれらを呼び出す場合は人間の承認ゲートを通過する必要があります。
...
NBRouteDecision[accessSpec]
(* <|"Route" -> "CloudLLM"|"PrivateLLM"|"LocalOnly",
     "EffectiveRiskScore" -> 0.3,
     "Thresholds" -> ...,
     "Reason" -> "..."|> *)
```
...
ClaudeRuntime の反復エージェントループをサポートする補助 API です。

```mathematica
(* 式が必要とするリソースを静的に推定します *)
NBInferExprRequirements[heldExpr, accessSpec]
(* <|"ReadCells" -> {...}, "WriteCells" -> {...},
     "RequiredAccessLevel" -> 0.5,
     "HasSideEffects" -> False, ...|> *)

(* 実行結果を指定 sink に安全に release します *)
NBReleaseResult[result, accessSpec]

(* 失敗情報から安全な retry packet を構築します *)
...
[ClaudeTestKit](https://github.com/transreal/ClaudeTestKit) は NBAccess および [claudecode](https://github.com/transreal/claudecode) の動作を検証するためのテストフレームワークです。ClaudeTestKit の導入により、NBAccess の各 API は以下の観点でテスト可能になっています。

### ClaudeTestKit が検証する NBAccess の主要機能

| テスト対象 | 使用する NBAccess API |
|---|---|
| プライバシーフィルタリング | `NBGetContext`, `NBCellPrivacyLevel`, `NBFilterCellIndices` |
| 機密マーク管理 | `NBMarkCellConfidential`, `NBMarkCellDependent`, `NBUnmarkCell` |
| 依存グラフの構築 | `NBBuildVarDependencies`, `NBTransitiveDependents`, `NBScanDependentCells` |
...
| SourceVault パス参照 | `NBNormalizePath`, `NBResolvePathRef`, `NBSetAccessiblePathRefs`, `NBGetAccessiblePathRefs` |

### テスト用ユーティリティとの連携

ClaudeTestKit はノートブックを開かずに NBAccess の動作を検証するため、主に以下のパターンを使用します。

```mathematica
(* テスト用の機密変数テーブルをセットアップします *)
NBSetConfidentialVars[<|"testSecret" -> True|>]

(* プライバシーレベルの判定を単体テストします *)
(* ... ClaudeTestKit のアサーション ... *)
...
ClaudeTestKit が NBAccess をテストする際、以下の点に注意が必要です。

- **FrontEnd 依存関数**: `NBCellReadInputText` や `NBWriteCode` 等の FrontEnd を必要とする関数は、ヘッドレス環境（FrontEnd なし）では動作しません。ClaudeTestKit はこれらの関数のテストを FrontEnd 付き環境でのみ実行します。
- **TaggingRules キャッシュ**: テスト間でキャッシュが残留しないよう、各テストケースの setUp/tearDown で `NBHistoryCacheClear[]` を呼び出すことを推奨します。SourceVault snapshot キャッシュをクリアする場合は `NBFileSpecCacheClear[]` も併用してください。
- **グローバル状態**: `$NBConfidentialSymbols`, `$NBPrivacySpec` 等のグローバル変数はテスト間で状態が共有されます。ClaudeTestKit は各テスト前後にこれらを保存・復元します。
...
ClaudeRuntime・ClaudeTestKit・SourceVault の導入にともなう NBAccess の変更は、既存のコードとの後方互換性を完全に維持するよう設計されています。

### 後方互換性の保証範囲

以下の既存 API はすべて変更なく動作し続けます。

- セル情報取得 API（`NBCellCount`, `NBCellStyle`, `NBCellLabel` 等）
- セル内容読み取り API（`NBCellRead`, `NBCellReadInputText`, `NBCellToText` 等）
- プライバシー制御 API（`NBCellPrivacyLevel`, `NBIsAccessible`, `NBGetContext` 等）
- 機密マーク管理 API（`NBMarkCellConfidential`, `NBMarkCellDependent`, `NBUnmarkCell` 等）
- 依存グラフ API（`NBBuildVarDependencies`, `NBScanDependentCells` 等）
- 書き込み API（`NBWriteCode`, `NBWriteText`, `NBWriteCell` 等）
- TaggingRules API（`NBGetTaggingRule`, `NBSetTaggingRule` 等）
- 履歴データベース API（`NBHistoryCreate`, `NBHistoryAppend` 等）
- Job 管理 API（`NBBeginJob`, `NBEndJob`, `NBAbortJob` 等）
- API キー・フォールバックモデル API
- アクセス可能ディレクトリ API（`NBSetAccessibleDirs`, `NBGetAccessibleDirs`）

### 新規追加 API の位置づけ

ClaudeRuntime・SourceVault 連携のために追加された以下の API 群は、既存の API に依存しない独立した拡張です。

- ラベル代数 API（`NBLabelQ`, `NBLabelJoin` 等）
- 認可ゲート API（`NBAuthorize`, `NBPolicyGate` 等）
- 関数セキュリティ API（`NBRegisterFunctionSecurity`, `GuardedApply` 等）
...
- SourceVault 連携 API（`NBNormalizePath`, `NBResolvePathRef`, `NBSetAccessiblePathRefs`, `NBGetAccessiblePathRefs`, `NBNormalizeAccessPathRef`, `NBSetSnapshotPrivacyLevel`, `NBFileSpecCacheClear`）

これらの新規 API は**オプトイン方式**です。明示的に呼び出さない限り、既存のワークフローには影響しません。

### SourceVault 未ロード環境での動作

SourceVault がロードされていない環境では、以下の挙動になります。

- `NBNormalizePath` は SymbolicPath を持たない簡易な PathRef を返します（`Kind`, `PhysicalPath`, `ResolutionStatus="ResolvedOnThisPC"|"Unrooted"` のみ）。
- `NBResolvePathRef` は PathRef の `PhysicalPath` をそのまま返します。
- `NBSetAccessiblePathRefs` は `claudeAccessiblePathRefs` キーに保存しますが、クロス PC 解決は行えません。
- `NBSetAccessibleDirs` / `NBGetAccessibleDirs` は完全に従来通り動作します。
- `NBSetSnapshotPrivacyLevel` は SourceVault が必要なため `$Failed` を返します。

SourceVault をロードすると、これらの API は自動的にクロス PC 解決やエイリアス対応に切り替わります。既存コードを書き換える必要はありません。

### 数値アクセスレベル体系の継続サポート

`AccessLevel` 数値（0.0〜1.0）によるプライバシー制御は、ClaudeRuntime 導入後も引き続き主要なアクセス制御機構として動作します。ラベル代数（`NBLabelQ` 等）は将来的に主体系となる予定ですが、現バージョンでは数値スコアと advisory 体系として**並列して**機能します。

```mathematica
(* 従来の数値ベース制御（引き続き動作します） *)
NBGetContext[nb, 5, PrivacySpec -> <|"AccessLevel" -> 0.5|>]
NBIsAccessible[nb, 3, PrivacySpec -> <|"AccessLevel" -> 0.8|>]

(* 新しいラベルベース制御（追加された API） *)
NBLabelLEQ[NBLabelBottom[], NBLabelTop[]]   (* True *)
NBCanFlowToQ[srcLabel, dstLabel]
```

### ラベルが未設定の場合のデフォルト動作

ラベルが明示的に設定されていないオブジェクトや関数に対しては、`NBLabelBottom[]`（最小制約 = public）がデフォルトとして適用されます。これにより、ラベル API を使用しない既存のコードは「全て公開データ」として扱われ、従来どおりに動作します。

同様に、`NBRegisterFunctionSecurity` で登録されていない関数は、`ExecPolicy -> "Open"` として扱われます。

### $NBSeparationIgnoreList の継続サポート

`$NBSeparationIgnoreList` の動作は変更されていません。NBAccess および NotebookExtensions は引き続きデフォルトで登録されており、`ClaudeCheckSeparation` の検査対象外です。
...
- [ClaudeRuntime](https://github.com/transreal/ClaudeRuntime) — NBAccess のセキュリティ基盤を利用した安全な実行環境
- [ClaudeTestKit](https://github.com/transreal/ClaudeTestKit) — NBAccess および claudecode の動作を検証するテストフレームワーク
- [SourceVault](https://github.com/transreal/SourceVault) — 複数 PC・複数ボールトを横断するパスのシンボリック管理とスナップショット
