...
- 生の `Import` の呼び出しは恒久的に Deny のままです。式中で `Import` を使いたい場合は、代わりに NBAccess が提供する `NBImport`（[NBAccess 経由のファイルインポート](#nbaccess-経由のファイルインポートnbimport新規追加)を参照）を使用してください。`$NBAllowedHeads`（層 1）には「代わりにこれを呼べ」と示す受け皿が用意されています。
...
### NBAccess 経由のファイルインポート（NBImport、新規追加）

NBAccess にアクセス制御・機密情報漏洩防止を統合した Import ラッパー **`NBImport`** が追加されました。ClaudeRuntime 経由で LLM が生成した式を実行する評価コンテキスト（`NBValidateHeldExpr`／`NBExecuteHeldExpr`）では、生の `Import` の呼び出しは恒久的に Deny されます。ファイルを読み込みたい場合は必ず `NBImport` を使用してください。

```mathematica
(* アクセス可能ディレクトリ内のファイルのみ読み込める *)
NBAccess`NBImport["C:\\path\\to\\dir\\data.csv", "CSV"]
```
...
```mathematica
(* アクセス可能ディレクトリ内のファイルを NBAccess 仲介で読み込む *)
NBAccess`NBImport["C:\\path\\to\\dir\\data.csv", "CSV"]
...
(* Detailed 版: ブロック区間・Mandatory フラグ込みで確認する *)
NBAccess`NBCalendarBusyQ[AbsoluteTime[], "Detailed" -> True]
...
zaiKey = NBAccess`NBGetAPIKey["zai"]
kimiKey = NBAccess`NBGetAPIKey["kimi"]
...
ClaudeRuntime 経由で実行される式（LLM 生成コードなど）の中で生の `Import` を呼び出すと、恒久的に Deny されます。これは意図した挙動です。代わりに `NBAccess`NBImport[path, fmt]` を使用してください（[NBAccess 経由のファイルインポート](#nbaccess-経由のファイルインポートnbimport新規追加)を参照）。旧来の `NBCheckedImport` を直接呼び出していたコードがある場合は、`NBImport` への置き換えが必要です。`NBImport` がアクセス拒否エラーを返す場合は、対象パスが `NBSetAccessiblePathRefs`（または `NBSetAccessibleDirs`）で宣言済みのアクセス可能ディレクトリに含まれているか確認してください。
...
イベントが 1 件も返らない、あるいはカレンダー全体が読めていないように見える場合は、`SystemCredential["ics-calendar"]`（または `$NBCalendarCredentialName` で指定したキー）にカレンダーの所在地（`.ics` ファイルパスまたは URL）が正しく設定されているかを確認してください。`NBCalendarBusyQ` はソースが読めない場合でも `Failure` ではなく `False`（`"Detailed" -> True` 指定時は `<|"Busy" -> False, "Reason" -> "SourceUnavailable"|>`）を返す（通知ゲーティングが「busy でない」側にフェイルオープンする）ため、常に `False`／`"SourceUnavailable"` が返る場合はソース未設定・読み取り不可を疑ってください。

`NBCalendarEvents` を `"Wrap" -> True` で呼んだ際に `"Completeness"` が 1 未満で返る場合は、`"MaxEvents"`（既定 500）による打ち切りが発生しています。対象期間の occurrence 数が多い場合は `"MaxEvents"` を大きくするか、期間を絞り込んでください。
