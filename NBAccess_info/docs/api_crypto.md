...
鍵を削除する。削除した keyRef は同一カーネルの merge 永続で復活しないよう内部 ledger
(`$iNBDeletedKeyRefs`)に記録される。

### NBKeyMaterialExistsQ[keyRef] → Boolean
鍵材料が backend に存在するかを **index 非依存**で返す。共有 index blob は複数カーネル間で
last-writer-wins の clobber が起きうるため、「鍵が本当に存在するか」(crypto-shredding 済み判定・
消去検査など)は NBKeyStatus でなく本関数を使う。

注: SystemCredential backend の鍵 index blob(`SourceVault__keyindex__v1`)は永続時に
