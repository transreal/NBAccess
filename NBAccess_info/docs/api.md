...
$onWork ノートブックのタスクメタデータを HELD 式(メタデータ Association を包む HoldComplete[...] または Hold[...]、あるいはその先頭要素がそれであるリスト)から一切評価せずに抽出する。ホワイトリストの文字列キー(Title/Status/Deadline/NextReview/EventDate/Keywords/Effort/Movable/DependsOn/TaskId)かつリテラル値(String/Integer/Real/True/False/DateObject[{ints..},(粒度)]/Quantity[num,unit]/文字列のリスト)のみを保持し、それ以外のキー・値は破棄する。副作用式・Notebook boxes・巨大な式・UpValue シンボルは決して評価しない。

### NBOnWorkTasks[opts] → List
$onWork の .nb ファイルを列挙し、NBOnWorkTaskSafeExtract 経由で読んだメタデータのみからアクセスレベル別に射影したタスクレコードを返す(本文/出力はどのレベルでも読まない)。各レコードは "Due"(Deadline、無ければ NextReview。NextReview が Quantity の場合は ModificationDate + オフセットとして解決、NotebookExtensions と同一挙動)と "State"(Status 由来の Done/Pass/Keep、無ければ Open)を導出する。アクセスレベル別フィールド: 0.5=Due/DueKind/State/FileDigest/ModificationDate、0.7 は Title/Keywords/TaskId/Effort/Movable/DependsOn を追加、1.0 は Path を追加。
Options: "Directory" -> Automatic (Global`$onWork), "ModifiedWithinDays" -> Automatic (全件), "IncludeDone" -> False (Status Done/Pass を除外), PrivacySpec -> Automatic, "MaxFiles" -> 2000, "Files" -> Automatic (テスト用シーム: `{<|"Path"->_, "Held"->HoldComplete[...], ("ModificationDate"->_)|>, ...}` でファイルシステムを迂回)
